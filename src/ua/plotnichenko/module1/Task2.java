package ua.plotnichenko.module1;
...
public class Task2 {
    ...
    public static String chessBoard ( int row , int column ) {
    ...
    public static void createBoard ( String[][] chessBoard ) {
        for (int i = 0; i < chessBoard.length; i++) {
            for (int j = 0; j < chessBoard[0].length; j++) {
                if (( i + j ) % 2 == 0) chessBoard[i][j] = chessBoard ( j , i );
                else chessBoard[i][j] = chessBoard ( j , i );
            }
        }
